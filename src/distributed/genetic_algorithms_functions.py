...
def calculate_fitness(route,
    ...
    # Validate the route: It must include every node exactly once.
    ...
    # Iterate through each consecutive pair in the route
    for i in range(len(route) - 1):
        node1 = route[i]
        node2 = route[i + 1]
        distance = distance_matrix[node1, node2] 
        # Return penalty if distance infeasible 
        if distance == 10000:
            return float(-1e6)
    ...
    # Return the negative total (because lower distance is better)
